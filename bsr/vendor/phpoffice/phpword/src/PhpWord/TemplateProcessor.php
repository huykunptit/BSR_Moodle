...
class TemplateProcessor
{
    ...
    public function cloneRowAndSetValues($search, $values)
    {
        ...
        // Set values in each cloned row
        foreach ($values as $rowKey => $rowData) {
            $rowNumber = $rowKey + 1;
    
            foreach ($rowData as $macro => $replace) {
                // Set the value, but skip empty strings for debugging
                if (!empty($replace)) {
                    $this->setValue($macro . '#' . $rowNumber, htmlspecialchars($replace, ENT_QUOTES, 'UTF-8'));
                }else{
                    $this->setValue($macro . '#' . $rowNumber, '');
                }
            }
        }
    }
