...
class grade_report_overview extends grade_report {
    ...
    public function __construct($userid, $gpr, $context) {
        ...
        // dd(($this->showtotalsifcontainhidden));
    ...
    public function setup_table() {
        ...
        if ($this->showrank['any']) {
            ...
            $tablecolumns = array('coursename', get_string('activitydate:opened', 'course') ,'grade');
            $tableheaders = array(grade_helper::get_lang_string('coursename' ,'grades'), get_string('activitydate:opened', 'course'),
                grade_helper::get_lang_string('gradenoun'));
        }
    ...
    public function fill_table($activitylink = false, $studentcoursesonly = false) {
        ...
        if ($this->courses) {
            ...
            foreach ($coursesdata as $coursedata) {
                ...
                $startdate = userdate($coursedata['course']->startdate, get_string('strftimedatefullshort', 'langconfig'), 0);
                ...
                $data = [$coursenamelink,$startdate, grade_format_gradevalue($finalgrade, $courseitem, true)];
...
function user_pdfexport_grade_profile($USER, $url, $exporthtml='') {
    global $DB, $CFG, $OUTPUT;

    $PDF_HEADER_LOGO = "pix/bsr_logo.png"; // Update with correct path to your logo image
    ...
    $pdf->Image($CFG->dirroot . '/' . $PDF_HEADER_LOGO, 10, 5, $PDF_HEADER_LOGO_WIDTH, '', 'PNG', '', 'T', true, 300, '', false, false, 0, false, false, false);
    $pdf->SetY(20);

    // Generate HTML content for PDF
    $html = user_display_grade_profile($USER, $url, $pdf, $exporthtml);
...
function user_display_grade_profile($user, $url, $pdf, $exporthtml) {
    global $DB, $CFG;

    // Fetch courses and grades
    $report = new grade_report_overview($user->id, null, context_user::instance($user->id));
    ...
    $html = '<h1>Course Grade Profile</h1>';
    $html .= '<h2>' . fullname($user) . '</h2>';
    $html .= '<table border="1">';
    $html .= '<tr><th>Course Name</th><th>Final Grade</th></tr>';
    foreach ($courseNames as $index => $courseName) {
        $html .= '<tr><td>' . $courseName . '</td><td>' . $finalGrades[$index] . '</td></tr>';
    }
    $html .= '</table>';

    // Add additional exported HTML content
    $html .= '<h3>Exported HTML:</h3>';
    $html .= $exporthtml;
