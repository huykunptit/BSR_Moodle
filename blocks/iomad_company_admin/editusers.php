...
if ($confirmuser and confirm_sesskey()) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $user_select = ($_POST['myoption'] == 'other') ? $_POST['otherreason'] : $_POST['myoption'];
        $user_id = $user->id; 
        $fieldid = 4;
        $select = "userid = :userid AND fieldid = :fieldid AND " . $DB->sql_compare_text('data') . " = :data";
        $params = array('userid' => $user_id, 'fieldid' => $fieldid, 'data' => $user_select);
        $existing_record = $DB->record_exists_select('user_info_data', $select, $params);
        if (!$existing_record) {
    ...
    if ($confirm != md5($suspend)) {
        ...
        echo '<form id="suspendForm" action="' . new moodle_url('editusers.php', $optionsyes) . '" method="post" class="custom-form">';
    
        echo '<p class="form-description">' . get_string('suspendcheckfull', 'block_iomad_company_admin', "'$fullname'") . '</p>';
        echo 'Chọn lí do đình chỉ:';
        echo '<select style="height: 60px;width: 350px;" name="myoption" id="myoption" class="custom-select">';
        ...
        echo '</select>';
        echo '<input type="text" name="otherreason" id="otherreason" style="display: none;" class="custom-input" />';
    
        echo '<input type="submit" value="' . get_string('confirm') . '" class="custom-button" />';
        echo '</form>';
        echo '<style>
        #suspendForm {
            display: flex;
    ...
    document.getElementById("suspendForm").addEventListener("submit", function() {
        var selectedOption = document.getElementById("myoption").value;
        if (selectedOption === "other") {
            var inputTextValue = document.getElementById("otherreason").value;
            document.getElementById("text1").value = inputTextValue;
        }
    });
    </script>';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $params = array('userid' => $user_id, 'fieldid' => $fieldid);

    // Kiểm tra xem dữ liệu đã tồn tại trong bảng user_info_data chưa
    $existing_record1 = $DB->record_exists_select('user_info_data', $select, $params);
    
    if ($existing_record1) {
        // Xóa bản ghi nếu tồn tại
        $DB->delete_records_select('user_info_data', $select, $params);
    }
}
