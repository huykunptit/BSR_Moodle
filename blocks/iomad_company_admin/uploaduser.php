...
$export_form = new admin_uploaduser_exportform();

if ($formdata = $export_form->get_data()) {
    ...
    if($formdata->export){
        ...
        $filename = $currentDate . '_MaPhBanChdanh_NhapNguoidung.xlsx';
...
if (!empty($cancelled)) {
    ...
    if (!empty($formdata->submitbutton)) {
        ...
        // positions
        $positions = $DB->get_record('user_info_field', array('shortname' => 'BSRPOS'));
        $positions_name = explode("\n", $positions->param1);
        $positions_code = explode("\n", $positions->param2);
        foreach ($positions_name as $key => $position) {
            $positions_value[str_replace("\r",'', $positions_code[$key])] = str_replace("\r", '', $position);
        }
        $positions_field_id = $positions->id;
...
//            dd($user);
            ...
            if ($existinguser) {
                ...
                if ($positions != false) {
                    ...
                    if(!empty($user->position_code) && array_key_exists($user->position_code, $positions_value)) {
                        $user_info_position = $DB->get_record('user_info_data', array('userid' => $user->id, 'fieldid' => $positions_field_id));
                        if($user_info_position) {
                            $DB->delete_records('user_info_data', array('userid' => $user->id, 'fieldid' => $positions_field_id));
                        }
                        $user_position = new stdClass();
                        $user_position->userid = $user->id;
                        $user_position->fieldid = $positions_field_id;
                        $user_position->data = $positions_value[$user->position_code];
                        $user_position->dataformat = 0;
                        try {
                            $DB->insert_record('user_info_data', $user_position);
                        } catch (dml_exception $e) {
                            $upt->track('position', get_string('invalidposition', 'block_iomad_company_admin'), 'error');
                            $upt->track('status', $strusernotaddederror, 'error');
                            $line[] = get_string('invalidposition', 'block_iomad_company_admin');
                            $errornum++;
                            $userserrors++;
                            $erroredusers[] = $line;
//                            $DB->delete_records('user', array('id' => $user->id));
                ...
                if(!empty($user->department_code)) {
                    ...
                    // OES: query to get the user's department
                    $department_from_code = $DB->get_record('department', array('company' => $company->id, 'shortname' => $user->department_code));
                    $user_department_user = new stdClass();
                    $user_department_user->userid = $user->id;
                    $user_department_user->departmentid = $department_from_code->id;
                    $user_department_user->companyid = $department_from_code->company;
                    try {
                        $existing_record = $DB->get_record('company_users', ['userid' => $user->id]);
                        if($existing_record) {
                            $DB->delete_records('company_users', ['userid' => $user->id]);
                        }
                        $DB->insert_record('company_users', $user_department_user);
                      
                        $user->department = $DB->get_record('department', array('id' => $department_from_code->parent))->name;
...
class uu_progress_tracker {
    ...
    public function init() {
        ...
        echo '<th class="header c'.$ci++.'" scope="col">Mã phòng ban</th>';
        echo '<th class="header c'.$ci++.'" scope="col">Mã chức danh</th>';
        echo '</tr>';
        $this->_row = null;
    }
...
function validate_user_upload_columns(&$columns) {
    ...
    // OES: for additional rows
    $stdfields[] = 'department_code';
    $stdfields[] = 'position_code';
