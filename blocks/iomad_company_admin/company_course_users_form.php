...
if ($coursesform->is_cancelled() || $usersform->is_cancelled() ||
    ...
    echo $output->header();
    $courseid=$courses;
    $first_course_id = reset($courseid);

    echo "<form action='/user/index.php' method='get'>";
    echo "<input type='hidden' name='id' value='$first_course_id'>";
    echo "<button type='submit' class='btn btn-primary'>Quay trở lại</button>";
    echo "</form>";
