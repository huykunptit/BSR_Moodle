...
class company_course_users_form extends moodleform {
    ...
    public function __construct($actionurl, $context, $companyid, $departmentid, $courses) {
        ...
        // $course = $DB->get_record('course', array('id' => $this->selectedcourses[0]));
        // dd($course);
        // $contextid = context_course::instance($this->selectedcourses[0]);
    ...
    public function create_user_selectors() {
        if (!empty ($this->selectedcourses)) {
            ...
            //    dd($this->potentialusers);
            }
            
            if (empty($this->currentusers)) {
                $this->currentusers = new current_company_course_user_selector('currentlyenrolldusers', $options);
    ...
    public function process() {
        ...
        if (optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
            ...
            // dd($this->potentialusers->get_selected_users());
            $add = true;
        }
                    ...
                    if ($allow) {
                        ...
                        foreach ($courses as $courseid) {
                            ...
                            company_user::enrol($adduser,
                                                ...
                                                //     if ($course === null) {
                                                //         dd(1);
                                                //         throw new Exception('Course object is null.');
                                                //     }
                                                //     if ($adduser === null) {
                                                //         dd(2);
                                                ...
                                                //     // Call the EmailTemplate::send method
                                                //     $a = EmailTemplate::send('user_added_to_course', array(
                                                //         'course' => $course,
                                                //         'user' => $adduser,
                                                //         'due' => $duedate
                                                //     ));
