...
class editusers_table extends table_sql {
    ...
    public function col_org($row) {
        global $DB, $USER, $company, $OUTPUT;

        $department_id = array_keys($DB->get_records('company_users', ['companyid' => $company->id, 'userid' => $row->id], '', 'departmentid'));
        $userdepartments = array_keys($DB->get_records('department', ['company' => $company->id, 'id' => $department_id[0]], '', 'parent'));
        if (empty($USER->editing) || $row->managertype == 1) {
            $count = count($userdepartments);
            $current = 1;
            $returnstr = "";
    ...
    public function col_avt($row) {
        $context = context_user::instance($row->id);
        $contextid = $context->id;
        return "<a href=\"/user/profile.php?id=$row->id\"><img src=\"/pluginfile.php/$contextid/user/icon/f1/\" alt=\"\" height='50px' class=\"userpicture\" /></a>";
    }

    public function col_suspended($row) {
        global $DB;
        $data = $DB->get_records('user_info_data', ['fieldid' => 4, 'userid' => $row->id], '', 'data');
