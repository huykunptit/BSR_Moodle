...
class profile_define_base {
     ...
     */
    public function define_form(&$form, $companyid=null, $isPos = false) {
        $form->addElement('header', '_commonsettings', get_string('profilecommonsettings', 'admin'));
        if($isPos) {
            $this->define_form_common_pos($form, $companyid);
     ...
     * @param   object   instance of the moodleform class
     */
    public function define_form_common(&$form, $companyid=null, $categoryid=null) {
        global $DB;
    ...
    /**
     * Prints out the form snippet for the part of creating or
     * editing a profile field common to all data types for position
     * @param   object   instance of the moodleform class
     */
    public function define_form_common_pos(&$form, $companyid=null, $categoryid=null) {
        global $DB;
        ...
        $form->setType('name', PARAM_MULTILANG);
        //disable name box
        $form->hardFreeze('name');
        $form->hardFreeze('shortname');
...
function profile_edit_field_pos($id, $datatype, $redirect, $companyid) {
    global $CFG, $DB, $OUTPUT, $PAGE;
    ...
    $field->description = clean_text($field->description, $field->descriptionformat);
    $field->description = array('text' => $field->description, 'format' => $field->descriptionformat, 'itemid' => 0);
    if ($field->shortname == "BSRPOS") {
        $isPos = true;
    } else {
        $isPos = false;
    }
    $fieldform = new \block_iomad_company_admin\forms\field_form(null, $field->datatype, $companyid, $isPos);
    // Convert the data format for.
    if (is_array($fieldform->editors())) {
        foreach ($fieldform->editors() as $editor) {
            if (isset($field->$editor)) {
    ...
    $positions_param1 = explode("\n", $field->param1);
    $positions_param2 = explode("\n", $field->param2);
    $positions_param1_str = "";
    $positions_param1_str_only = "";
    foreach ($positions_param1 as $key => $position) {
        $positions_param1_str_only .= $position . "\n";
        $positions_param1_str .= $position . "|" . $positions_param2[$key] . "\n";
    }
    $field->param1 = $positions_param1_str;
    $fieldform->set_data($field);
    ...
    if ($fieldform->is_cancelled()) {
        ...
        if ($data = $fieldform->get_data()) {
            ...
            $param1 = "";
            $param2 = "";
            $raw_param = $data->param1;
            $position_values = explode("\n", $raw_param);
            foreach($position_values as $position_value) {
                $position_extract = explode("|", $position_value);
                $param1 .= $position_extract[0] . "\n";
                if(count($position_extract) > 1 || $position_extract[1] != "") {
                    $param2 .= $position_extract[1] . "\n";
                } else {
                    $param2 .= visanitize($position_extract[0]) . "\n";
                }
            }
            $data->param1 = $param1;
            $data->param2 = $param2;
        ...
        if($isPos) {
            ...
            $table->head = array('Chức danh', 'Mã');
            $table->data = array();
            $positions = explode("\n", $positions_param1_str_only);
            $positions_code = explode("\n", $field->param2);
            foreach ($positions as $key => $position) {
                $table->data[] = array($position, $positions_code[$key]);
            }
