...
class admin_uploaduser_form1 extends company_moodleform {
    ...
    public function definition () {
        ...
        $mform =& $this->_form;
        $url1 = new moodle_url('BSR_CODE_DEPARTMENT_POSITION.xlsx');
        $url = new moodle_url('BSR_User_Upload_Template.csv');
        // $mform->addElement('static', 'examplecsv', get_string('uploadusertemplate', 'block_iomad_company_admin'), null, array('class' => 'inline'));
        $currentDate = date('dmy');
        $filename = $currentDate . '_Filemau_NhapNguoidung.csv';
        $fileUrl = new moodle_url('/blocks/iomad_company_admin/BSR_User_Upload_Template.csv'); 
        $buttons = array(
            $mform->createElement('button', 'example_button', get_string('download', 'block_iomad_company_admin'), array(
        ...
        $mform->addGroup(
            $buttons,
            'button_group', 
            get_string('uploadusertemplate', 'block_iomad_company_admin'), 
            array(' '), 
            false 
        );
        echo ' 
...
class admin_uploaduser_exportform extends moodleform {
    public function definition() {
        global $CFG, $USER;
        $mform = $this->_form; // Initialize the form object

        // Add submit button directly to the form
        // $mform->addElement('submit', 'export', get_string('download', 'block_iomad_company_admin'));

        // Optionally, you can group multiple elements if needed, but here it's just a button
        // You can remove this line if you don't need to group elements
        $mform->addGroup(array($mform->createElement('submit', 'export', get_string('download', 'block_iomad_company_admin'))), 
            'button_group', 
            get_string('departmentandpositoncode', 'block_iomad_company_admin'), 
            array(' '), false);
    }
}
