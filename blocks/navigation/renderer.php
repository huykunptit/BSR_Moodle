...
class block_navigation_renderer extends plugin_renderer_base {
    ...
    protected function navigation_node($items, $attrs=array(), $expansionlimit=null, array $options = array(), $depth=1) {
        ...
        $keysToRemove = [
            'myhome', '1', 'frontpageloaded', 'siteblog', '4', 'notes', 
            'tags', 'privatefiles', 'contentbank', 'courseblogs', 'competencies','badgesview'
        ];
        
        $filteredItems = [];
        foreach ($items as $item) {
            if (!in_array($item->key, $keysToRemove)) {
                $filteredItems[] = $item; // Add to filtered array if not in keysToRemove
            }
        }
        foreach ($filteredItems as $item) {
            ...
            $liattr = ['class' => [$item->get_css_type(), 'depth_'.$depth], 'role' => 'treeitem','data-loaded' => 'false'];
            ...
            if ($isbranch) {
                ...
                if ($item->requiresajaxloading) {
                    $dataLoaded = $item->collapse ? 'false' : 'true';
                    $liattr += [
