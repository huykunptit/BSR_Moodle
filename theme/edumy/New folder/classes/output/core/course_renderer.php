        ...
        function buildHtmlList($tree, $prefix = '') {
            ...
            foreach ($tree as $key => $value) {
                if (is_array($value)) {
                    ...
                    $html .= '<a href="/course/index.php?categoryid=' . $id . '" for="' . $prefix . $key . '">' . $key . '</a>';
                    $html .= '</summary>';
    ...
    //     ';
        ...
        if ($coursecat->id && $coursecat->is_uservisible()) {
            if (isset($PAGE->theme->settings->courseliststyle) && ($PAGE->theme->settings->courseliststyle == 2)) {
                ...
                $tree = buildTree($select->options);
                $finalTree = buildHtmlList($tree);
                $name = get_string('selectacategory', 'question');
                $output .= '<div class="d-flex m-2 justify-content-between">
                    <div class="col-lg-1">
                        <div class="instructor_search_result style2">
                            <p class="mt10 fz15">' . $ccnCourseCountRender . '</p>
                        </div>
                    ...
                    <div class="col-lg-11">
                        <div class="candidate_revew_select style2 text-right mb25 easyui-panel" style="padding: 5px;">
                            <ul class="mb0">
                                <li class="list-inline-item">' . $this->render($select) . '</li>
                                <button id="showTreeButton" style="color:white;padding:10px;background: #1e4fa5;border-radius: 0.4em;border:solid #276ef8">' . $chooseCategory . '</button>
                                <li class="list-inline-item">' . $this->course_search_form() . '</li>
                            </ul>
                        </div>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Ensure that treeModal and showTreeButton exist
            if (showTreeButton && treeModal) {
                ...
                // Close the modal when the close button is clicked, if closeBtn exists
                if (closeBtn) {
                ...
                console.error("showTreeButton or treeModal element not found.");
            }
        });
...
//    private function buildTree($options) {
//        $tree = [];
//
//        foreach($options as $id => $option) {
//            $option = str_replace(' / ', '/', $option);
//            $num_slashes = substr_count($option, '/');
//            $tmp_space = str_repeat('&nbsp;', $num_slashes * 3);
//
//            // Kiểm tra xem nút có con không
//            $has_children = false;
//            foreach($options as $child_option) {
//                if(strpos($child_option, $option . '/') === 0) {
//                    $has_children = true;
//                    break;
//                }
//            }
//
//
//            if($num_slashes > 0 && $has_children) {
//                $tmp_space .= '+ ';
//            }
//
//            $option = $tmp_space . basename($option);
//            $tree[$id] = $option;
//        }
//
//        return $tree;
//    }
        ...
        <div class="col-lg-12 p0"><div class="courses_list_content">
         <div class="top_courses list ccnWithFoot">
           ...
           <div class="details">
            ...
            $content .= '
          <div class="col-lg-2 col-md-4 col-sm-4 col-6" style="width: 60%;">
            ...
            $content .='
                         ...
                         <div class="tc_footer">
                           <ul class="tc_meta float-left">
                             ...
                             <li class="list-inline-item" style="font-size:15px;" ><i class="flaticon-book"></i>'.$ccn_items_count.'</li>
                           </ul>
                         </div>
    ...
    protected function coursecat_subcategories(coursecat_helper $chelper, $coursecat, $depth) {
        ...
        foreach ($subcategories as $subcategory) {
            if($subcategory->can_create_course() || $subcategory->has_manage_capability())
