...
//            dump($question1);
            $counttext1 = isset($question1->answer) && (is_array($question1->answer) || is_countable($question1->answer)) ? count($question1->answer) : 0;
            // dd($question1);
            $question_found_in_db = false;

            // Kiểm tra xem câu hỏi đã có trong DB chưa
            foreach ($question_db_check as $question_check) {
                if ($question_check->questiontext == $question1->questiontext) {
                    $is_similar[$key1] = 1;
                    $question_found_in_db = true;
                    echo "<div class='questionblock'><hr style='background-color: #D37676;' /><p style='background-color: #D37671; padding: 10px;color:aliceblue;border-radius: 40px;'><b style='color:aliceblue'> Câu hỏi:</b> {$question1->name}: {$question1->questiontext} đã có trong ngân hàng câu hỏi.</p></div>";
                    break;
                }
            }


            if (!$question_found_in_db) {
                foreach ($questions as $key2 => $question2) {
                    $counttext2 = isset($question2->answer) && (is_array($question2->answer) || is_countable($question2->answer)) ? count($question2->answer) : 0;

                    if ($key1 != $key2 && $question1->questionname === $question2->questionname && $question1->questiontext === $question2->questiontext) {
                        $found = false;
                        foreach ($similar_questions as $similar_question) {

                            if ($similar_question['question1']['questiontext'] === $question1->questiontext && $similar_question['question2']['questiontext'] === $question2->questiontext) {
                                $found = true;
                                break;
                            }
                        }
                        if (!$found) {
                            // Thêm cặp câu hỏi vào mảng $similar_questions
                            $similar_questions[] = [
                                'question1' => [
                                    'name' => $question1->name,
        ...
        if($is_similar[$key1===2]) {
            echo "<b>Đã có câu trùng nhau, vui lòng kiểm tra lại:</b> <br>";
            $printed_questions = [];


            foreach ($similar_questions as $similar_question_pair) {
                var_dump($similar_question_pair);
                $question1_name = $similar_question_pair['question1']['name'];
                $question2_name = $similar_question_pair['question2']['name'];

                if (!in_array($question1_name, $printed_questions)) {
                    $question1_text = $similar_question_pair['question1']['questiontext'];
                    echo "<b>Câu hỏi:</b> $question1_name: $question1_text <br>";
                    echo "Đáp án: ";
                    foreach ($questions as $question) {
                        if ($question->name == $question1_name) {
                    ...
                    $printed_questions[] = $question1_name;
                }
                if (!in_array($question2_name, $printed_questions)) {
                    $question2_text = $similar_question_pair['question2']['questiontext'];
                    echo "<b>Câu hỏi:</b> $question2_name: $question2_text <br>";
                    echo "Đáp án: ";
                    foreach ($questions as $question) {
                        if ($question->name == $question2_name) {
                            foreach ($question->answer as $answer) {
                                echo $answer['text'] . ",";
                            }
                            echo "<br>";
                    ...
                    $printed_questions[] = $question2_name;
                }
            }
...
//        End check
        ...
        foreach ($questions as $key => $question) {   // Process and store each question
