...
function get_users($get=true, $search='', $confirmed=false, array $exceptions=null, $sort='firstname ASC',
    ...
    if ($extraselect) {
        $extraselect = str_replace('AND', 'OR', $extraselect);
    ...
    foreach ($params as $key => $value) {
        if (strpos($key, 'ex_text') !== false) { 
            $filter = true;
            if (!$first) {
                // Add 'OR' for subsequent clauses
                $sql .= " OR ";
            }
            $value = "'$value'";
            $value = str_replace('%', '', $value);
            // Append the LIKE clause
            $sql .= "name LIKE N$value";
            $first = false; // Set this to false after the first iteration
        }
    }
    if($filter) {
        $departments = $DB->get_records_sql($sql, $params);
        $parent_ids = array_keys($departments); 

    }

    $related_departments = [];
    // Get all ids from the $departments array
    if (!empty($parent_ids)) {
        // Step 2: Build SQL query using WHERE parent IN (...)
        list($in_sql, $in_params) = $DB->get_in_or_equal($parent_ids);
        
        $sql = "SELECT * FROM {department} WHERE parent $in_sql";

        // Step 3: Execute the query
        $related_departments = $DB->get_records_sql($sql, $in_params);

        // Debug/Output related departments
    } 

    $department_ids = array_keys($related_departments);  // Get all ids from $related_departments array
    $extended_users = [];
    if (!empty($department_ids)) {
        // Step 2: Build SQL query using WHERE department_id IN (...)
        list($in_sql, $in_params) = $DB->get_in_or_equal($department_ids);

        $sql = "SELECT userid FROM {company_users} WHERE departmentid $in_sql";

        // Step 3: Execute the query
        $extended = $DB->get_records_sql($sql, $in_params);
        $user_id = array_keys($extended);
        list($in_sql, $in_params) = $DB->get_in_or_equal($user_id, SQL_PARAMS_NAMED);
        $sql = "SELECT * FROM {user} WHERE id $in_sql";
        $extended_users = $DB->get_records_sql($sql, $in_params);
        
        // Debug/Output the users
    }

    $extended_user_ids = array_map(function($user) {
        return $user->id;
    }, $extended_users);
    $or_where = '';
    if (!empty($extended_user_ids)) {
        list($in_sql, $in_params) = $DB->get_in_or_equal($extended_user_ids, SQL_PARAMS_NAMED, 'extended');
        $or_where = " OR id $in_sql";
    }
    $select .= $or_where;

    if ($get) {
        if($in_params)
            return $DB->get_records_select('user', $select, array_merge($params, $in_params), $sort, $fields, $page, $recordsperpage);
        else 
            return $DB->get_records_select('user', $select, $params, $sort, $fields, $page, $recordsperpage);
    } else {
        if($in_params)
            return $DB->count_records_select('user', $select, array_merge($params, $in_params));
        else
...
function get_users_listing($sort='lastaccess', $dir='ASC', $page=0, $recordsperpage=0,
    ...
    $sql = "SELECT u.id $selects
            FROM {user} u
                $joins
        WHERE $select $or_where
        $sort";
    // dd($sql);
    if($in_params)
        $users = $DB->get_records_sql($sql, array_merge($params, $joinparams, $in_params), $page, $recordsperpage);
    else
        $users = $DB->get_records_sql($sql, array_merge($params, $joinparams), $page, $recordsperpage);
    }
    catch (Exception $e) {
        dd($e);
    }

            // Merge extended users with the main query result
    $merged_users = $extended_users + $users; // Combine arrays, prioritize extended users
...
function get_courses_search($searchterms, $sort, $page, $recordsperpage, &$totalcount,
    ...
    if ($DB->get_dbfamily() == 'oracle') {
        $concat = "(c.fullname || ' ' || c.shortname)";
    } else {
        $concat = $DB->sql_concat('c.fullname', "' '", 'c.shortname');
    }
    // dd($concat);
