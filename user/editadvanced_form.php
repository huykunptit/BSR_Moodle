...
class user_editadvanced_form extends moodleform {
    ...
    public function definition() {
        ...
        $userid = $user->id;
        global $DB;
        $position = $DB->get_record('user_info_data', array('userid' => $userid,'fieldid' => 1));
        ...
        // Shared fields.
        // dd($user->id);
        $user_department = $DB->get_record('company_users', array('userid' => $user->id))->departmentid;
      
        useredit_shared_definition($mform, $editoroptions, $filemanageroptions, $user);
       // Add the text input for 'department_id' with the 'd-none' class
        $mform->addElement('text', 'department_id', get_string('department_id', 'your_plugin'), array('class' => 'd-none'));

        // Set the default value for 'department_id' to $user_department
        $mform->setDefault('department_id', $user_department);

        // Optionally, you can set the type to PARAM_INT if department_id is numeric
        $mform->setType('department_id', PARAM_INT);

        // Add the text input for 'position_data' with the 'd-none' class
        $mform->addElement('text', 'position_data', get_string('position'), array('class' => 'd-none'));

       
        $mform->setType('position_data', PARAM_RAW);
        // $mform->addElement('button', 'openModalBtn', 'Chọn Phòng/Ban');
        // Nex  t the customisable profile fields.
        profile_definition($mform, $userid);
        // dd($mform,$user,$editoroptions,$filemanageroptions);
        if ($userid == -1) {
    ...
    //    dd($user,$mform,$editoroptions,$filemanageroptions);
//        dd($this);
        
        $this->set_data($user);
        // dd($this->set_data($user));
    }
