...
function user_create_user($user, $updatepassword = true, $triggerevent = true) {
    ...
    $department = $DB->get_record('department', ['id' => $user->department_id]);
    $user->department = $department->name;
    // Insert the user into the database.
    $newuserid = $DB->insert_record('user', $user);
    // $new_record = new stdClass();
    //         $new_record->userid = $newuserid;
    //         $new_record->fieldid = 1;
    //         $new_record->data = $user->position_data; // Use position_data from the user.
    //         $DB->insert_record('user_info_data', $new_record);
    //         // dd($new_record); // Check the new record before inserting   
    if (!empty($user->position_data)) {
    //    dd($user->position_data);    
        // Check if a position record already exists for this user.
        $position = $DB->get_record('user_info_data', ['userid' => $newuserid, 'fieldid' => 1]);
        if (empty($position)) {
            try{
                $new_record = new stdClass();
                $new_record->userid = $newuserid;
            ...
            // Update the existing position record.
           
       
        }
    } else {
        debugging("Position data is missing for user: {$newuserid}", DEBUG_DEVELOPER);
    }

       
        $company_user = new stdClass();
        $company_user->companyid = 2;
        $company_user->userid = $newuserid;
        $company_user->manager_type = 0;
        if( $user->department_id != null &&  $user->department_id != "[[department_id]]"){
        $company_user->departmentid = $user->department_id;
      
        } else {
            $company_user->departmentid = 1;
        }
        $company_user->suspended = 0;
        $company_user->educator = 0;
        $existing_record = $DB->get_record('company_users', ['userid' => $newuserid]);
            // If no existing record is found, insert the new record
        if (!$existing_record) {
            $newcompanyuserid = $DB->insert_record('company_users', $company_user);
        } 
...
function user_update_user($user, $updatepassword = true, $triggerevent = true) {
    ...
    // Update or insert into user_info_data (position data)
    $position = $DB->get_record('user_info_data', ['userid' => $currentrecord->id, 'fieldid' => 1]);
    // dd($position);
    if (($position)) {
        $position->data = $user->position_data;
        // dd($position);
        if ($DB->update_record('user_info_data', $position)) {
            // dd(1);
            debugging("Position data updated successfully", DEBUG_DEVELOPER);
        } else {
            debugging("Failed to update position data", DEBUG_DEVELOPER);
        }
    } 
    // else {
    //     dd(1);
    //     $new_record = new stdClass();
    //     $new_record->userid = $currentrecord->id;
    //     $new_record->fieldid = 1;
    //     $new_record->data = $user->position_data;
    //     if ($DB->insert_record('user_info_data', $new_record)) {
    //         debugging("Position data inserted successfully", DEBUG_DEVELOPER);
    //     } else {
    //         debugging("Failed to insert position data", DEBUG_DEVELOPER);
    //     }
    // }
    
    // Handle department update in company_users
    $department = $DB->get_record('department', ['id' => $user->department_id]);
    ...
    if ($company_user) {
        if ($user->department_id != "[[department_id]]") {
            if ($user->department_id !== null) {
                ...
                $company_user->departmentid = 1; // Default department if not set
            }
        ...
        $company_user->suspended = 0;
        $company_user->educator = 0;
        $existing_record = $DB->get_record('company_users', ['userid' => $user->id]);   
        if (!$existing_record) {
            $newcompanyuserid = $DB->insert_record('company_users', $company_user);
            $DB->update_record('user', $user);
    ...
    // Update user attributes only if there are changes
    if (!empty($changedattributes)) {
        $changedattributes['timemodified'] = time(); // Update timemodified
...
function user_get_default_fields() {
    return array( 'id', 'username', 'fullname', 'firstname', 'lastname', 'email','ban','phong',
