...
function useredit_shared_definition(&$mform, $editoroptions, $filemanageroptions, $user) {
    ...
    $mform->addElement('text', 'department', get_string('department'), array(
        'maxlength' => 255,
        'size' => 25,
        'value' => $user->department // Set value to the user's department
    ));
    ...
    $mform->addElement('button', 'openModalBtn', get_string('choosedepartment','moodle'));
    // $mform->addElement('text', 'position', get_string('position'), 'maxlength="255" size="25"');
    // $mform->addElement('button',get_string('search_pOSITION'));

    $positionsString = $DB->get_field('user_info_field', 'param1', array('shortname' => 'BSRPOS'));
$positions = array_map('trim', explode("\n", $positionsString)); 
sort($positions);
$user_position = $DB->get_field('user_info_data', 'data', array('userid' => $user->id, 'fieldid' => 1));
// dd($user_position);
$mform->addElement('text', 'position', get_string('position','filters'), array(
    'id' => 'positionInput', 
    'maxlength' => 255, 
    'size' => 25, 
    'value' => $user_position
));
$mform->hardFreeze('position');
$mform->addElement('button', 'searchButton', get_string('searchposition','moodle'), array('type' => 'button', 'id' => 'searchButton1'));
...
function selectPosition1(position) {
    // Set the value of the input field with id "positionInput"
    document.getElementById("positionInput").value = position;
    
    // Set the same value to the input field with id "id_position_data"
    document.getElementById("id_position_data").value = position;
    
    // Log the position value for debugging
    console.log("Position selected:", position);
    
    // Hide the modal with id "positionModal1"
