...
class user_filter_text extends user_filter_type {
    ...
    public function setupForm(&$mform) {
        ...
        $filter_data = $this->check_data($field);
//        dd($field, $filter_data);
        $attributes = array('class' => 'd-none');
        $objs['select'] = $mform->createElement('select', $this->_name.'_op', null, $this->getOperators(),$attributes);
        $objs['text'] = $mform->createElement('text', $this->_name, null);
        if ($this->_name === 'department') {
            // Create the text element
           
        
            // Create the button elements
            $openModalButton = $mform->createElement('button', 'openModalBtn', get_string('choosedepartment','moodle'));
            $searchButton = $mform->createElement('button', 'searchButton', get_string('searchposition','moodle'), array('type' => 'button', 'id' => 'searchButton1'));
        
            // Group the buttons together within a div
            $buttonArray = [];
            $buttonArray[] = $mform->createElement('html', '<div class="form-group row"><div class="col p-0">');
             $objs['text'] = $mform->createElement('text', 'department', get_string('valuefor', 'filters', $this->_label), array('id' => 'selectedDepartment', 'readonly' => 'readonly'));
             
            //  <input type="text" class="form-control " name="profile" id="id_profile" value="">
            $buttonArray[] = $openModalButton;
            $buttonArray[] = $mform->createElement('html', '</div><div class="col p-0">');
            $buttonArray[] = $searchButton;
            $buttonArray[] = $mform->createElement('html', '</div></div>');
        
            // Add the grouped buttons to the form
...
//dump($objs['text']);
        ...
        $objs['text']->setLabel(get_string('valuefor', 'filters', $this->_label),$attributes);
