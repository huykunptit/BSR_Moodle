...
class user_filter_profilefield extends user_filter_type {
    ...
    public function setupForm(&$mform) {
        ...
        $chucdanh = $profilefields[1]; 
        $filteredProfileFields = array(1 => $chucdanh);
        if (empty($filteredProfileFields)) {
            return;
        }
        $objs = array();
        $attributes = array('class' => 'd-none');
        $objs['field'] = $mform->createElement('select', $this->_name.'_fld', null, $filteredProfileFields,$attributes);
       
        $objs['op'] = $mform->createElement('select', $this->_name.'_op', null, $this->get_operators(),$attributes);
        $objs['value'] = $mform->createElement('text', $this->_name, null);
        $objs['field']->setLabel(get_string('position', 'filters',$attributes));
        $objs['op']->setLabel(get_string('profilefilterlimiter', 'filters',$attributes));
