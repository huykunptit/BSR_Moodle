...
class enrol_manual_potential_participant extends user_selector_base {
     ...
     * @return array
     */
    public function find_users($search, $departmentId = 0) {
        global $DB;
       ...
       if($departmentId > 0) {
            $availableusers = $this->enrol_manual_get_users_by_department($availableusers, $departmentId);
        }
        ...
        if(!$change_search && !$change_department) {
    ...
    private function enrol_manual_get_users_by_department(array $availableusers, mixed $departmentId)
    {
        global $DB;
        ...
        // get users by department from table mdl_company_users
        $sql = "SELECT u.id, u.firstname, u.lastname, u.email, u.picture, u.imagealt, u.department
        FROM {user} u
        JOIN {company_users} cu ON (cu.userid = u.id)
        WHERE u.id IN ($userIds) AND cu.departmentid = :depid";


//        $params = array('departmentId' => $departmentId);

        $users = $DB->get_records_sql($sql, array('depid' => $departmentId));
