...
//         $check_question = $DB->get_records('question', ['name' => $form->name]);
//         //var_dump($check_question);
//         $found_similar = 0;
//         foreach ($check_question as  $question1) {
// //            var_dump($question1);
//             $question_found_in_db = false;
//             $check_answer = $DB->get_records('question_answers', ['question' => $question1->id]);
// //            var_dump($check_answer); exit(0);
//             $num_answers_form = 0;
//             foreach ($form->answer as $answer) {
//                 // Kiểm tra nếu trường text không rỗng
//                 if (!empty(trim($answer['text']))) {
//                     $num_answers_form++;
//                 }
//             }
// //            var_dump($num_answers_form); exit(0);
// //                var_dump($form->answer); exit(0);
//             // Kiểm tra xem $check_answer có khác null không
//             if ($check_answer !== null) {
//                 $num_answers_check = count($check_answer);
// //                var_dump($num_answers_check);exit(0);
//                 if ($num_answers_form == $num_answers_check) {
//                     $count = 0;
//                     foreach ($form->answer as $answer_question) {
// //                        var_dump($answer_question['text']); exit(0);
//                         foreach ($check_answer as $answer_check) {
// //                            var_dump($answer_check->answer);exit(0);
//                             if($answer_question['text'] == $answer_check->answer) {
//                                 $count++;
//                             }
//                         }
//                     }

// //                     if ($count == $num_answers_form) {
// //                         $found_similar = 1;
// // //                        var_dump($form->cmid);
// //                         $_SESSION['alert_message'] = "Đã có câu này";
// //                         $linkurl = new moodle_url('/mod/quiz/edit.php?cmid=' . $form->cmid . '/moodle_iomad/mod/quiz/edit.php?cmid=' . $form->cmid);
// //                         echo "<script type='text/javascript'>alert('Câu hỏi này đã được thêm, vui lòng kiểm tra lại !'); window.location.href='$linkurl';</script>";exit();
// //                         sleep(8);
...
//            var_dump($question_extension_table);
    ...
    protected function import_or_save_files($field, $context, $component, $filearea, $itemid) {
        if (!empty($field['itemid'])) {
            // This is the normal case. We are saving the questions editing form.
            $text = isset($field['text']) ? $field['text'] : '';
            return file_save_draft_area_files($field['itemid'], $context->id, $component,
                $filearea, $itemid, $this->fileoptions, trim($text));
