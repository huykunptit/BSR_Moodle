...
$categoryid = $_POST['categorymoveto_'] ?? optional_param('category', 0, PARAM_INT);
...
if(!$categoryid){
    $categoryid = optional_param('category_', 0, PARAM_INT);
}
...
if ($mform->is_cancelled()) {
   ...
   $fromform->categorymoveto = $categoryid;
    if ($makecopy) {
        $question->id = 0;
    ...
    if (isset($fromform->categorymoveto) && $fromform->categorymoveto != $question->category) {
        list($newcatid, $newcontextid) = explode(',', $fromform->categorymoveto);
        // dd($newcatid,$newcontextid);
        if (!empty($question->id) && $newcatid != $question->categoryobject->id) {
            $contextid = $newcontextid;
            question_require_capability_on($question, 'move');
        } else {
