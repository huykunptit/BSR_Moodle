...
require_once($CFG->dirroot . '/question/bank/importquestions/classes/form/import_form_category.php');
require_once($CFG->dirroot . '/question/format.php');
require_once($CFG->dirroot . '/question/renderer.php');

use qbank_importquestions\form\question_import_form_category;
use context;

require_login();
core_question\local\bank\helper::require_plugin_enabled('qbank_importquestions');
list($thispageurl, $contexts, $cmid, $cm, $module, $pagevars) =
        question_edit_setup('import', '/question/bank/importquestions/import1.php');
// Get display strings.
$txt = new stdClass();
...
$importform = new question_import_form_category($thispageurl, ['contexts' => $contexts->having_one_edit_tab_cap('import'),
...
list($catid, $catcontext) = explode(',', reset($importform->getCategory()));
if (!$category = $DB->get_record("question_categories", ['id' => $catid])) {
    throw new moodle_exception('nocategory', 'question');
}
// Page header.
...
if ($form = $importform->get_data()) {
    // File checks out ok.
    // dd($form->isAllowDupplicate?true:false);
    $fileisgood = false;
    ...
    $qformat->setAllowDuplicate($form->isAllowDupplicate?true:false);
    ...
    echo $OUTPUT->continue_button(new moodle_url('/question/edit1.php', $params));
    echo $OUTPUT->footer();
    exit;
}
