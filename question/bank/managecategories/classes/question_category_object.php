...
class question_category_object {
    ...
    public function initialize($page, $contexts, $currentcat, $defaultcategory, $todelete, $addcontexts): void {
        ...
        $this->catform = new question_category_edit_form($this->pageurl,
                ['contexts' => $contexts, 'currentcat' => $currentcat ?? 0, 'defaultcategory' => $defaultcategory ?? 0]);
        if (!$currentcat) {
    ...
    public function output_edit_lists(): void {
        ...
        foreach ($this->editlists as $context => $list) {
            $course_cat = $DB->get_record('context', array('id' => $context));
            try {
                $cat = core_course_category::get($course_cat->instanceid);
            } catch (Exception $e) {
                continue;
            }
            if (!$cat->can_create_course()) {
            ...
            $listhtml = $list->to_html(1, ['str' => $this->str]);
            $listhtml = str_replace('<ul>', '<ul class="collapsible-list">', $listhtml);
        
            if ($listhtml) {
                // Create a unique ID for each box
                $uniqueid = 'boxwidthwide-' . $context;
        
                echo $OUTPUT->box_start('boxwidthwide boxaligncenter generalbox questioncategories contextlevel' .
                    $list->context->contextlevel, $uniqueid);
                ...
                // CSS
                echo '<style>
                .boxwidthwide .collapsible-list {
                    list-style-type: none;
                    padding-left: 20px;
                }
                
                .boxwidthwide .collapsible-list li {
                    margin-bottom: 5px;
                    position: relative;
                }
                
                .boxwidthwide .collapsible-list ul {
                    display: none;
                }
                
                .boxwidthwide .toggle-btn {
                ...
                .boxwidthwide .toggle-btn:hover {
                    background-color: #e0e0e0;
                    color: black;
                }
                
                /* Styling for distinguishing classes */
                .boxwidthwide .has-sublist {
                    font-weight: bold; /* Example: make items with sublists bold */
                }
                
                .boxwidthwide .last-level {
                    color: gray; /* Example: color last-level items gray */
                }
                </style>';
                ...
                (function() {
                    ...
                    var lists = box.querySelectorAll(".collapsible-list");
                    ...
                    lists.forEach(function(list) {
                        ...
                        items.forEach(function(item) {
                            ...
                            var existingToggle = item.querySelector(".toggle-btn");
                
                            // Check if the li contains a ul
                            if (subList) {
                                item.classList.add("has-sublist"); // Add class to distinguish
                
                                // Only add a new toggle button if none exists
                                if (!existingToggle) {
                                ...
                                item.classList.add("last-level"); // Class for last-level li elements
                            }
                        });
                    });
                })();
