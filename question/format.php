...
function get_category_path($categoryId) {
    ...
    while ($currentCategoryId) {
        ...
        $category = $DB->get_record('question_categories', ['id' => $currentCategoryId], 'id, name, parent', MUST_EXIST);

        // Prepend the category name to the path array
    ...
    // Join the path array with " / " to create the full path string
    return implode(' / ', $path);
}
...
class qformat_default
{
    ...
    public function setCategory($category)
    {
        ...
        $this->importcontext = context::instance_by_id(
            $this->category->contextid
        );
    }
    ...
    public function setContexts($contexts)
    {
        ...
        $this->translator = new core_question\local\bank\context_to_string_translator(
            $this->contexts
        );
    }
    ...
    /**
     * Set whether to allow duplicate questions
     * @param bool $isAllowDuplicate
     */
    public function setAllowDuplicate($isAllowDuplicate)
    {
     ...
     */
    public function try_importing_using_qtypes(
        $data,
        $question = null,
        $extra = null,
        $qtypehint = ""
    ...
    public function importprocess()
    {
        ...
        if ($this->displayprogress) {
            echo $OUTPUT->notification(
                get_string("parsingquestions", "question"),
                "notifysuccess"
            );
        }
        ...
        if (!($questions = $this->readquestions($lines))) {
            ...
            echo $OUTPUT->notification(
                get_string("noquestionsinfile", "question")
            );
            return false;
        }
        ...
        if ($this->displayprogress) {
            echo $OUTPUT->notification(
                get_string(
                    "importingquestions",
                    "question",
                    $this->count_questions($questions)
                ),
                "notifysuccess"
            );
        }
        ...
        if ($this->stoponerror and $this->importerrors > 0) {
            echo $OUTPUT->notification(
                get_string("importparseerror", "question")
            );
            return true;
        }
        ...
        $gradeerrors = 0;
        $goodquestions = [];
            $path = get_category_path($this->category->id);
        ...
        foreach ($questions as $question) {
            if (!empty($question->fraction) and is_array($question->fraction)) {
                ...
                foreach ($fractions as $key => $fraction) {
                    $newfraction = match_grade_options(
                        $gradeoptionsfull,
                        $fraction,
                        $this->matchgrades
                    );
        ...
        // Check Similar Question - huynq
        //        $question_db_check = $DB->get_records('question');
        //        var_dump($question111s->questiontext);
      ...
      WHERE 
        ...
        foreach ($result as $question) {
            if (!empty($question->id)) {
                $question_db_check[$question->id] = $question;
            }
        }
        $result->close();
        // dd($question_db_check);
        // dd($question_db_check[1]);
        // dd($questions);
        foreach ($questions as $key1 => $question1) {
            $is_similar[$key1] = $is_similar[$key1] ?? 0;
            $counttext1 =
                isset($question1->answer) &&
                (is_array($question1->answer) ||
                    is_countable($question1->answer))
                    ? count($question1->answer)
                    : 0;
            $question_found_in_db = false;
            // dd($questions);
            // Check if the question already exists in the database
            foreach ($question_db_check as $question_check) {
                //check if the question is similar to the question in the database
                if (
                    $question1->qtype != "category" &&
                    $question_check->questiontext == $question1->questiontext &&
                    $question_check->name == $question1->name
                ) {
                    // variable to check similar answers
                    $is_answers_similar = true;
                    // Check if the question is multichoice
                    if (
                        $question_check->qtype == "multichoice" &&
                        $question1->qtype == "multichoice"
                    ) {
                        // Get the answers of the question
                        $question_db_check_answers = $DB->get_records(
                            "question_answers",
                            ["question" => $question_check->id]
                        );
                        // Check if the number of answers is the same
                        if (
                            count($question1->answer) ==
                            count($question_db_check_answers)
                        ) {
                            // Create arrays of answers with their fractions
                            $answers1 = [];
                            $answers2 = [];

                            // answers1 is array of question input's answers
                            foreach ($question1->answer as $key => $answer) {
                                $answers1[] = [
                                    "text" => $answer["text"],
                                    "fraction" => round(
                                        $question1->fraction[$key],
                                        1
                                    ),
                                ];
                            }
                            //answers2 is array of question in database's answers
                            foreach (
                                $question_db_check_answers
                                as $key => $answer
                            ) {
                                $answers2[] = [
                                    "text" => $answer->answer,
                                    "fraction" => round($answer->fraction, 1),
                                ];
                            }
                            // Sort the answers arrays
                            usort($answers1, function ($a, $b) {
                                $textComparison = strcmp(
                                    $a["text"],
                                    $b["text"]
                                );
                                if ($textComparison == 0) {
                            ...
                            foreach ($answers1 as $key => $answer) {
                                if (
                                    $answer["text"] !=
                                        $answers2[$key]["text"] ||
                                    $answer["fraction"] !=
                                        $answers2[$key]["fraction"]
                                ) {
                                    //set check variable to false if its has answer not similar
                                    $is_answers_similar = false;
                                    break;
                                }
                            }
                        }
                    ...
                    if (
                        $question_check->qtype == "match" &&
                        $question1->qtype == "match"
                    ) {
                        // Get the answers of the question
                        $question_db_check_answers = $DB->get_records(
                            "qtype_match_subquestions",
                            ["questionid" => $question_check->id]
                        );
                        // dump($question1);
                        // dump($question_check);
                        // dump($question_db_check_answers);
                        // Check if the number of answers is the same
                        if (
                            count($question1->subquestions) ==
                            count($question_db_check_answers)
                        ) {
                            // Create arrays of answers with their fractions
                            $answers1 = [];
                            ...
                            // answers1 is array of question input's answers
                            foreach (
                                $question1->subquestions
                                as $key => $answer
                            ) {
                                $answers1[] = [
                                    "text" => $answer["text"],
                                    "answer" => $question1->subanswers[$key],
                                ];
                            }
                            //answers2 is array of question in database's answers
                            foreach (
                    ...
                    //if the answers is smilar then it is duplicate
                    if ($is_answers_similar) {
                        $is_similar[$key1] = 1;
                        $question_found_in_db = true;
                        echo "<div class='questionblock'><hr style='background-color: #D37676;' /><p style='background-color: #D37671; padding: 10px;color:aliceblue;border-radius: 40px;'><b style='color:aliceblue'> Câu hỏi:</b> <span class='questionname'>{$question1->name}</span>: <span class='questiontext'>{$question1->questiontext}</span> đã có trong ngân hàng câu hỏi <b><i>".$path.".</i></b></p></div>";
                        break;
                    }
                }
            }
            ...
            if (
                !$question_found_in_db &&
                $question1->qtype != "description" &&
                $question1->qtype != "category"
            ) {
                for ($key2 = $key1 + 1; $key2 < count($questions); $key2++) {
                    $question2 = $questions[$key2];
                    // dump($question1);
                    if (isset($question2->qtype)) {
                        $dupplicate_answer = $this->areQuestionsSimilar(
                            $question1,
                            $question2
                        );

                        // dd($dupplicate_answer);
                        if (
                            ...
                            $is_similar[$key1] = 2;
                            $is_similar[$key2] = 2;
                            $is_similar_check = true;
                            // dump($question2);
                            $found = false;
                            foreach ($similar_questions as $similar_question) {
                                if (
                                    $similar_question["question1"][
                                        "questiontext"
                                    ] === $question1->questiontext &&
                                    $similar_question["question2"][
                                        "questiontext"
                                    ] === $question2->questiontext &&
                                    $similar_question["question1"]["name"] ===
                                        $question1->name &&
                                    $similar_question["question2"]["name"] ===
                                        $question2->name
                                ) {
                                    $found = true;
                                    break;
        ...
        // dump($is_similar);
        // dd(1);
        // In ra các câu trùng nhau
        if ($is_similar_check) {
            $printed_questions = [];

            foreach ($is_similar as $key => $similar) {
                $q = $questions[$key];
                // dump($q);
                if ($similar === 2 && $q->qtype != "category") {
                    // dump($q);
                    echo $this->isAllowDuplicate
                        ? ""
                        : "<div class='questionblock'><hr style='background-color: #D95A6A;' /> <p class='mt-4' style='background-color: #D95A6A; padding: 10px;color:aliceblue;border-radius: 40px;'><b>Câu hỏi:</b> <span clas='questionname'>$q->name:</span></p> <strong> $q->questiontext </strong>câu hỏi trùng lặp trong file <br></div>";
                    // echo "Đáp án: ";
                    // echo "</br>";
        ...
        // dump($is_similar);
        // dd(array_filter($questions, function ($q) {
        //     return $q->qtype != 'category';
        // }));
        $is_has_similar = false;
        ...
        foreach ($questions as $key => $question) {
            ...
            if (
                $is_similar[$key] === 0 ||
                ($is_similar[$key] === 2 && $this->isAllowDuplicate)
            ) {
                $count++;
                if ($this->displayprogress) {
                    echo "<div class='questionblock12312312312312312312312'><hr style='background-color: #1e4fa5;' /><p style='background-color: #1e4fa5; padding: 10px;color:aliceblue;border-radius: 40px;'><b style='color:aliceblue'> Câu hỏi {$count}:</b> 
                    </p>
                    <strong> " .
                        $this->format_question_text($question) .
                        " </strong>
                    <br> 
                    Import câu hỏi thành công vào ngân hàng  <b>".$path."</b>
                    </div>";
                }
            }
            ...
            if ($is_similar[$key] != 0 && $question->qtype != "category") {
                $is_has_similar = true;
            }

            if (
                ($is_similar[$key] === 0 && $question->qtype != "category") ||
                ...
                $questionbankentry->id = $DB->insert_record(
                ...
                // dd($questionversion);
                $questionversion->id = $DB->insert_record(
                    "question_versions",
                    $questionversion
                );
                ...
                } elseif (isset($question->questiontextfiles)) {
                ...
                } elseif (isset($question->generalfeedbackfiles)) {
                ...
                // dump($this->questionids);
                // Now to save all the answers and type-specific options

                $result = question_bank::get_qtype(
                    $question->qtype
                ...
                if (core_tag_tag::is_enabled("core_question", "question")) {
                    ...
                    $importingcoursecontext = $importingcontext->get_course_context(
                        false
                    );
                    $isimportingcontextcourseoractivity = !empty(
                        $importingcoursecontext
                    );

                    if (!empty($question->coursetags)) {
                        if ($isimportingcontextcourseoractivity) {
                            $mergedtags = array_merge(
                                $question->coursetags,
                                $question->tags
                            );
        ...
        if ($is_has_similar) {
            echo '
            <button onclick="exportQuestionsToCSV()" class="btn btn-danger" id="exportButton"> Tải danh sách câu hỏi lỗi </button>

            <script src="' .
                $CFG->wwwroot .
                '/theme/edumy/javascript/html2canvas.min.js"></script>
            <style>
            th {
                width: 30%
            }
            ...
            #overview-grade td, #overview-grade th {
            ...
            function exportQuestionsToCSV() {
                ...
                csvRows.push(["Danh sách câu hỏi lỗi khi nhập"]);
                csvRows.push([]);
                
                // Add header row
                csvRows.push(["STT", "Tên câu hỏi", "Nội dung câu hỏi", "Loại lỗi"]);
                
                let index = 1;
                questionBlocks.forEach((block) => {
                    const errorParagraph = block.querySelector("p[style*=\"background-color\"]");
                    if (errorParagraph) {
                        const backgroundColor = errorParagraph.style.backgroundColor;
                        if (backgroundColor === "rgb(211, 118, 113)" || backgroundColor === "rgb(217, 90, 106)") {
                            const questionName = block.querySelector(".questionname")?.textContent.trim() || "N/A";
                            const questionText = block.querySelector(".questiontext")?.textContent.trim() || 
                                                 block.querySelector("strong")?.textContent.trim() || "N/A";
                            
                            let errorType = "";
                            if (backgroundColor === "rgb(211, 118, 113)") {
                                errorType = "Đã có trong ngân hàng câu hỏi";
                            } else if (backgroundColor === "rgb(217, 90, 106)") {
                                errorType = "Trùng lặp trong file";
                            }
                            
                            const escapedQuestionText = "\"" + questionText.replace(/"/g, "\"\"") + "\"";
                            
                            csvRows.push([index, questionName, escapedQuestionText, errorType]);
                            index++;
                        }
                    }
                });
                ...
                if (csvRows.length <= 3) {
                    alert("Không có câu hỏi lỗi để xuất.");
                    return;
                }
                ...
                // Set the download attribute with the date prefix
                a.download = dateString + "_DS_cau_hoi_loi.csv";
            ...
            async function exportToImage() {
                ...
                try {
                    const targetDiv = document.querySelector(".our-dashbord.dashbord.ccn_dashbord_disable_drawer");
            
                    if (!targetDiv) {
                        throw new Error("Không tìm thấy phần tử để chụp ảnh");
                    }
                    ...
                    // Create a new canvas with A4 aspect ratio
                    const a4Ratio = 297 / 210; // height / width
                    let newWidth, newHeight;
            
                    if (canvas.height / canvas.width > a4Ratio) {
                        newHeight = canvas.height;
                        newWidth = canvas.height / a4Ratio;
                    } else {
                        newWidth = canvas.width;
                        newHeight = canvas.width * a4Ratio;
                    }
            
                    const a4Canvas = document.createElement("canvas");
                    a4Canvas.width = newWidth;
                    a4Canvas.height = newHeight;
                    ...
                    // Draw the content onto the A4 canvas
                    const scale = Math.min(newWidth / canvas.width, newHeight / canvas.height);
                    const x = (newWidth - canvas.width * scale) / 2;
                    const y = (newHeight - canvas.height * scale) / 2;
                    ...
                    const filename = `${day}${month}${year}_Loi_nhap_cau_hoi.png`;
            ...
            echo '
            <button onclick="resubmitForm()" class="btn btn-primary">Cho phép nhập câu hỏi trùng lặp</button>
            <script>
            function resubmitForm() {
                $(\'[id*="mform1_"] input[name="isAllowDupplicate"]\').val(1);
                $(\'[id*="mform1_"] input[type="submit"]\').click();
            }
            </script>';
        }
    ...
    function areQuestionsSimilar($question1, $question2)
    {
        if (
            $question1->qtype == "multichoice" &&
            $question2->qtype == "multichoice"
        ) {
            if (count($question1->answer) == count($question2->answer)) {
                // Create arrays of answers with their fractions
                $answers1 = [];
                $answers2 = [];
                ...
                foreach ($question1->answer as $key => $answer) {
                    $answers1[] = [
                        "text" => $answer["text"],
                        "fraction" => $question1->fraction[$key],
                    ];
                }

                foreach ($question2->answer as $key => $answer) {
                    $answers2[] = [
                        "text" => $answer["text"],
                        "fraction" => $question2->fraction[$key],
                    ];
                }

                // Sort the answers arrays
                usort($answers1, function ($a, $b) {
                    $textComparison = strcmp($a["text"], $b["text"]);
                    if ($textComparison == 0) {
                        return $a["fraction"] <=> $b["fraction"];
                    }
                    return $textComparison;
                ...
                foreach ($answers1 as $key => $answer) {
                    if (
                        $answer["text"] != $answers2[$key]["text"] ||
                        $answer["fraction"] != $answers2[$key]["fraction"]
                    ) {
                        return false;
                    }
                }
    ...
    function checkDuplicateAnswers(
        ...
        foreach ($answers1 as $answer1_key => $answer1) {
            foreach ($answers2 as $answer2_key => $answer2) {
                if (
                    $answer1 == $answer2 &&
                    $fractions1[$answer1_key] == $fractions2[$answer2_key]
                ) {
                    return true;
                }
            }
    ...
    protected function create_category_path($catpath, $lastcategoryinfo = null)
    {
        ...
        if (
            isset($catnames[0]) &&
            ($catnames[0] != "top" || count($catnames) < 3)
        ) {
        ...
        foreach ($catnames as $key => $catname) {
            if ($parent == 0) {
                ...
                if ($key == count($catnames) - 1 && $lastcategoryinfo) {
                    ...
                    if (
                        isset($lastcategoryinfo->info) &&
                        $lastcategoryinfo->info !== "" &&
                        $category->info === ""
                    ) {
                        $category->info = $lastcategoryinfo->info;
                        if (
                            isset($lastcategoryinfo->infoformat) &&
                    ...
                    // Same for idnumber.
                    if (
                        isset($lastcategoryinfo->idnumber) &&
                ...
                $parent = $category->id;
                $event = \core\event\question_category_created::create_from_question_category_instance(
                    $category,
                    $context
                );
                $event->trigger();
    ...
    protected function readquestion($lines)
    {
        ...
        throw new coding_exception(
            "Question format plugin is missing important code: readquestion."
        );
     ...
     */
    protected function try_exporting_using_qtypes(
        $name,
        $question,
        $extra = null
    ) {
    ...
    public function exportprocess($checkcapabilities = true)
    {
        ...
        if ($this->category) {
            ...
            $questions = get_questions_category(
                $this->category,
                true,
                true,
                true,
        ...
        foreach ($questions as $question) {
            ...
            if ($this->cattofile) {
                ...
                $trackcategoryparents = question_categorylist_parents(
                    $trackcategory
                );
                ...
                foreach ($trackcategoryparents as $trackcategoryparent) {
                    ...
                    if (!in_array($trackcategoryparent, $writtencategories)) {
                        ...
                        if (
                            !count(
                                $DB->get_records("question_bank_entries", [
                                    "questioncategoryid" => $trackcategoryparent,
                                ])
                            )
                        ) {
                            ...
                            $categoryinfo = $DB->get_record(
                                "question_categories",
                                ["id" => $trackcategoryparent],
                                "name, info, infoformat, idnumber",
                                MUST_EXIST
                            );
                            if ($categoryinfo->name != "top") {
                                // Create 'dummy' question for parent category.
                                $dummyquestion = $this->create_dummy_question_representing_category(
                                    $categoryname,
                                    $categoryinfo
                                );
                                $expout .=
                                    $this->writequestion($dummyquestion) . "\n";
                ...
                if (
                    $addnewcat &&
                    !in_array($trackcategory, $writtencategories)
                ) {
                    $categoryname = $this->get_category_path(
                        $trackcategory,
                        $this->contexttofile
     ...
     */
    protected function create_dummy_question_representing_category(
        string $categoryname,
        $categoryinfo
    ) {
    ...
    protected function get_category_path($id, $includecontext = true)
    {
        ...
        $contextstring = $this->translator->context_to_string(
            $category->contextid
        );
    ...
    protected function writequestion($question)
    {
        ...
        throw new coding_exception(
            "Question format plugin is missing important code: writequestion."
        );
        return null;
    }
