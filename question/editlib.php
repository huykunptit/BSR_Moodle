...
function question_edit_setup($edittab, $baseurl, $requirecmid = false, $unused = null) {
    ...
    $params['searchtext'] = optional_param('searchtext', null, PARAM_TEXT);
    // Category list page.
    $params['cpage'] = optional_param('cpage', null, PARAM_INT);
    $params['qtagids'] = optional_param_array('qtagids', null, PARAM_INT);

    $PAGE->set_pagelayout('admin');
    $course_cat = core_course_category::get_all();
    if(false) {
    
        foreach ($course_cat as $cat) {
            if ($cat->can_create_course()) {
                $course = $DB->get_record('course', array('category' => $cat->id));
                $context = $DB->get_record('context', array('contextlevel' => 40, 'instanceid' => $cat->id));
                $ques_cat = $DB->get_record('question_categories', array('contextid' => $context->id));
                $params['courseid'] = $course->id;
                $params['cat'] = $ques_cat->id;
                $params['cat'] .= ', ' . $context->id;
                break;
            }
        }
    }
...
function question_build_edit_resources($edittab, $baseurl, $params,
    ...
    $searchtext = $cleanparams['searchtext'];
    ...
    // $defaultcategory = question_make_default_categories($contexts->all()); //hoangledztest
    $contextlistarr = [];
    ...
    if (!empty($pagevars['cat'])){
        ...
        $course_id = $courseid;
        $course_category = $DB->get_record('course', array('id' => $course_id), 'category');
        $context = $DB->get_record('context', array('contextlevel' => 40, 'instanceid' => $course_category->category));
        $all_question_categories = $DB->get_records('question_categories', array('contextid' => $context->id));
        // // last category
        // $last_category = end($all_question_categories);
        // dd($all_question_categories);
        // $pagevars['cat'] = "{$last_category->id},{$last_category->contextid}";
        // $pagevars['cat'] = "{$category->id},{$category->contextid}";
        $output_cate = $defaultcategory;
        $output_cate = "{$defaultcategory->id},{$defaultcategory->contextid}";
        foreach($all_question_categories as $category) {
            if($category->parent == 0) {
                $output_cate = "{$category->id},{$category->contextid}";
                break;
            }
        }
        

        $pagevars['cat'] = $output_cate;
    }

    // Display options.
    $pagevars['searchtext']    = question_set_or_get_user_preference('searchtext', $searchtext, "bsr", $thispageurl);
