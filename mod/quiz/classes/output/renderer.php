...
class renderer extends plugin_renderer_base {
    ...
    public function access_messages($messages) {
        ...
        foreach ($messages as $message) {
            $parts = explode(':', $message, 2); // Split the message into two parts at the first colon
            if (count($parts) == 2) {
                $boldPart = html_writer::tag('span', $parts[0] . ':', ['class' => 'fw-bold']);
                $normalPart = html_writer::tag('span', $parts[1], ['class' => 'text-main fw-bold']);
                $output .= html_writer::tag('p', $boldPart . $normalPart, ['class' => 'text-left']);
            } else {
                // If there's no colon, treat the whole message as normal
