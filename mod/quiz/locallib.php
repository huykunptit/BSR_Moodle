...
//    $eventdata->quizid            = $a->quizid;

    $eventdata->userfrom          = core_user::get_noreply_user();
    $eventdata->userto            = $recipient;
    $eventdata->subject           = get_string('emailconfirmsubject', 'quiz', $a);
    $attempt_grade = $DB->get_record('quiz_attempts', array('id' => $a->attemptid))->sumgrades;
    $a->attemptgrade = round($attempt_grade,2);
    $quiz_grade = $DB->get_record('quiz', array('id' => $a->quizid))->grade;
    $a->quizgrade = round($quiz_grade,2);
    $quizObject = $DB->get_record('quiz', array('id' => $a->quizid));
    $grade_pass = grade_pass($quizObject);
    $a->grade_pass = round($grade_pass,2);

    if($a->attemptgrade >= $a->grade_pass){
        $a->passfail = "Đạt";
    } else {
        $a->passfail = "Không đạt";
    }
...
//    $quizGrades = $DB->get_records('quiz_grades', array('quiz' => $quiz->id));
    ...
    if ($studentisonline) {
        ...
        if($a->passfail== "Đạt"){
            $eventdata->fullmessage = 'Chào {$a->username},

            Cám ơn bạn đã hoàn thành bài thi "{$a->quizname}" của kỳ thi  "{$a->coursename}" vào {$a->submissiontime}.
            
            Email này xác nhận rằng bài làm của bạn đã được nộp vào hệ thống thành công!
            
            Bạn có thể truy cập bài làm  tại {$a->quizurl}. Nếu bạn không thể truy cập được bài thi thì có thể bạn chưa được phân quyền.
            
            -- Hệ thống thi trực tuyến ETest --';
        } else {
            $eventdata->fullmessage = 'Chào {$a->username},
...
//        $eventdata->fullmessage = "Quiz ID: " . $a->quizid . " Quiz Name: " . $a->quizname . " Quiz URL: " . $a->quizurl;
