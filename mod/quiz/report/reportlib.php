...
function quiz_report_grade_method_sql($grademethod, $quizattemptsalias = 'quiza') {
    switch ($grademethod) {
        case QUIZ_GRADEHIGHEST:
            // Assuming 'gradepass' is a column, make sure it's included in the SELECT list
...
function grade_pass($quiz) {
    global $DB;
    $conditions = array(
        'courseid' => $quiz->course,
        'itemname' => $quiz->name,
    );
    $grade_pass = $DB->get_record('grade_items', $conditions);
    $gradepass=$grade_pass->gradepass;
   
    return $gradepass;
}


function text_result($quiz, $grade) {
    $gradepass=grade_pass($quiz);
    if ($grade >= $gradepass) {
        $result = get_string('pass', 'quiz');
    } else {
        $result = get_string('fail', 'quiz');
    }
    return $result;
}
/**
