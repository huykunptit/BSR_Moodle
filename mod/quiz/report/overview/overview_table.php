...
class quiz_overview_table extends attempts_report_table {
    ...
    public function build_table() {
       ...
       // Lấy dữ liệu quiz_grades từ database
$quizGrades = $DB->get_records('quiz_grades', array('quiz' => $this->quiz->id));

// dump($quizGrades);

// Chuyển đổi $quizGrades thành một mảng liên kết với userid làm khóa
$gradesArray = [];
foreach ($quizGrades as $grade) {
    $gradesArray[$grade->userid] = $grade->grade;
}

// Chuyển đổi rawdata thành mảng
$rawdataArray = json_decode(json_encode($this->rawdata), true);
$grade_pass = grade_pass($this->quiz);
$quizGrades = $DB->get_records('quiz_grades', array('quiz' => $this->quiz->id));
// dump($quizGrades);
...
if ($this->quiz->grade >= $grade_pass) {
    foreach ($rawdataArray as $key => $row) {
        $userid = $row['userid'];
        if (isset($gradesArray[$userid])) {
            $row['grades'] = $gradesArray[$userid];
        } else {
            $row['grades'] = null; // Giá trị mặc định nếu không có grade
        }

        $row['gradepass'] = quiz_format_grade($this->quiz, $grade_pass);
        
        // So sánh điểm grades với gradepass
        if ($row['grades'] < $grade_pass) {
            $row['result'] = 'K.Đạt';
        } else {
            $row['result'] = 'Đạt';
        }

        $rawdataArray[$key] = $row;
    }
}
        ...
        parent::build_table($this->rawdata);
        $quizGrades = $DB->get_records('quiz_grades', array('quiz' => $this->quiz->id));
        ...
        if ($quizGrades) {
            foreach ($quizGrades as $quizGrade) {
                ...
                if ($userRecord) {
                    ...
                    $user_position = $DB->get_record('user_info_data', array('userid' => $userId, 'fieldid' => 1))->data;
                    $resultArray = [
                        'name'       => $user_name,
                        'department' => $user_department,
                        'position'   => $user_position,
                        'grade'      => $user_grade,
                        'result'     => $user_grade >= $this->quiz->gradepass ? 'Đạt' : 'K.Đạt',
                    ];
                    $resultArrays[] = $resultArray;
                }
            }
        } else {
            echo "Không tìm thấy thông tin điểm thi.";
        }
        $this->add_separator();
    ...
    public function compute_average_row($label, \core\dml\sql_join $usersjoins) {
       ...
       $averagerow = [
           ...
           'gradepass' => quiz_format_grade($this->quiz, grade_pass($this->quiz)),
           'result' => $user_grade >= $this->quiz->gradepass ? 'Đạt' : 'K.Đạt',
