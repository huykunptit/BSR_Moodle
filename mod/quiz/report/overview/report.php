...
class quiz_overview_report extends attempts_report {
    ...
    public function display($quiz, $cm, $course) {
        global $DB, $PAGE, $quizobj, $CFG, $values;
//        dd($this->rawdata);
        $this->exporttitle = get_string('overviewreport', 'quiz_overview');
...
//            dd($headers);
            ...
            $columns[] = 'gradepass';
            $headers[] =  get_string('gradepass', 'grades' );
            $columns[] ='result';
            $headers[] =  get_string('result', 'grades' );
...
//                dd($data,$bandwidth,$bands);
    ...
    function test($fields, $query, $params, $count, $file=null)
    {
        $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor($CFG->dirroot . '/bsr/resources/BSR-HRM-PRO-002-F-004-2 Ket qua dao tao Rev 7.docx');
        $templateProcessor->setValue('tieude', $this->exporttitle);
    ...
    function download_bsr_hrm_pro_002_f_004_2_rev7($fields, $query, $params, $count, $file=null) {
        global $DB, $CFG;

        $diemtoithieudat = optional_param('diemtoithieudat', '0', PARAM_TEXT);
        if (empty($diemtoithieudat)) {
            $diemtoithieudat = '0';
        }
        if (strstr($diemtoithieudat, ',')) {
            $diemtoithieudat = str_replace(',', '.', $diemtoithieudat);
        }
        ...
        $diemindex = 6;
        foreach ($fields as $key => $head) {
            if (trim($head->heading) == 'Điểm kỳ thi') {
                $diemindex = $key;
                break;
            }
        }
        //print_object($fields);die;
        $values = array();
        ...
        if ($records = $DB->get_recordset_sql($query, $params)) {
            foreach ($records as $record) {
                ...
                $dat = false;
                if (!empty($diem)) {
                    $dat = floatval(str_replace(',', '.', $diem)) >= floatval($diemtoithieudat);
                }
        ...
        $fullname = strtolower(stripVN($this->exporttitle));
        $filename = clean_filename(substr(preg_replace(array('/[^a-zA-Z\d\s-_]/', '/[\s-]/'), array('', '_'), $fullname), 0, 45) . '_BSR-HRM-PRO-002-F-004-2 Ket qua dao tao Rev 7.docx');
        //print_object($filename);die;
